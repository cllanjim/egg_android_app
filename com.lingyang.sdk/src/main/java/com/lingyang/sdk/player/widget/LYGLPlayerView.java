...
//import com.lingyang.sdk.player.GLPlayerRenderer;
 ...
 */
class LYGLPlayerView extends GLSurfaceView {

    private static final boolean DEBUG = false;
    private ACPlayer mPlayer;

    public LYGLPlayerView(Context context) {
        super(context);
    }

    public LYGLPlayerView(Context context, boolean translucent, int depth, int stencil) {
        super(context);
    }
    ...
    private static void checkEglError(String prompt, EGL10 egl) {
        ...
        while ((error = egl.eglGetError()) != EGL10.EGL_SUCCESS) {
            CLog.e(String.format("%s: EGL error: 0x%x", prompt, error));
        }
    }
    ...
    private static class ContextFactory implements EGLContextFactory {
        ...
        public EGLContext createContext(EGL10 egl, EGLDisplay display, EGLConfig eglConfig) {
            CLog.w("creating OpenGL ES 2.0 context");
    ...
    private static class ConfigChooser implements EGLConfigChooser {
        ...
        private void printConfigs(EGL10 egl, EGLDisplay display, EGLConfig[] configs) {
            ...
            CLog.w(String.format("%d configurations", numConfigs));
            for (int i = 0; i < numConfigs; i++) {
                CLog.w(String.format("Configuration %d:\n", i));
        ...
        private void printConfig(EGL10 egl, EGLDisplay display, EGLConfig config) {
            int[] attributes = {EGL10.EGL_BUFFER_SIZE, EGL10.EGL_ALPHA_SIZE, EGL10.EGL_BLUE_SIZE,
                    EGL10.EGL_GREEN_SIZE, EGL10.EGL_RED_SIZE, EGL10.EGL_DEPTH_SIZE,
                    EGL10.EGL_STENCIL_SIZE, EGL10.EGL_CONFIG_CAVEAT, EGL10.EGL_CONFIG_ID,
                    EGL10.EGL_LEVEL, EGL10.EGL_MAX_PBUFFER_HEIGHT, EGL10.EGL_MAX_PBUFFER_PIXELS,
            ...
            for (int i = 0; i < attributes.length; i++) {
                ...
                if (egl.eglGetConfigAttrib(display, config, attribute, value)) {
                    CLog.w(String.format("  %s: %d\n", name, value[0]));
                } else {
                    // CLog.w( String.format("  %s: failed\n", name));
